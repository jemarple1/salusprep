...
@section('content')
            ...
            @foreach ($scenario['sections'] as $key => $label)
                <div>
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <span class="text-xs text-slate-500 soap-count" data-section="{{ $key }}">0</span>
                    </div>
                    <div class="soap-zone min-h-[5.5rem] space-y-2 rounded-2xl border border-white/10 bg-navy/50 p-3 transition" data-section="{{ $key }}">
                        <p class="soap-placeholder text-center text-xs text-slate-600">Drop statements here</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="mt-8 flex flex-wrap items-center gap-4">
        <button type="button" id="soap-check" class="rounded-xl bg-medic px-8 py-3 font-bold text-white hover:bg-medic-dark">Check answers</button>
        <button type="button" id="soap-reset" class="rounded-xl border border-white/10 px-6 py-3 font-semibold text-slate-300 hover:bg-white/5">Reset</button>
        <p id="soap-result" class="text-sm font-semibold"></p>
    </div>

    <script>
        (function () {
            var pool = document.getElementById('soap-pool');
            var initialPoolHtml = pool.innerHTML;

            function zones() { return Array.prototype.slice.call(document.querySelectorAll('.soap-zone')); }
            function chips() { return Array.prototype.slice.call(document.querySelectorAll('.soap-chip')); }

            function updatePlaceholders() {
                zones().forEach(function (zone) {
                    var placeholder = zone.querySelector('.soap-placeholder');
                    var hasChips = zone.querySelector('.soap-chip');
                    if (placeholder) placeholder.style.display = hasChips ? 'none' : 'block';
                    var counter = document.querySelector('.soap-count[data-section="' + zone.dataset.section + '"]');
                    if (counter) counter.textContent = String(zone.querySelectorAll('.soap-chip').length);
                });
            }
            ...
            function bindDropTarget(target) {
                ...
                target.addEventListener('drop', function (e) {
                    ...
                    var chip = document.querySelector('.soap-chip[data-id="' + e.dataTransfer.getData('text/plain') + '"]');
            ...
            document.getElementById('soap-check').addEventListener('click', function () {
                var placements = {};
                chips().forEach(function (chip) {
                    var zone = chip.closest('.soap-zone');
