...
@section('content')
    <div class="mb-8">
        ...
        <p class="mt-2 max-w-2xl text-slate-400">
            @if ($sectionSlug === 'nclex-pn')
                Hands-on NCLEX-PN scenarios for prioritization, ADPIE, delegation, isolation precautions, medication safety, therapeutic communication, and clinical scales.
            @else
    ...
    @if ($exercises !== [])
        <x-exercise-grid
            :exercises="$exercises"
            title="All exercises"
            description="{{ $sectionSlug === 'nclex-pn' ? 'Practice nursing judgment with interactive drills — 10 scenarios per level, 5 levels each.' : 'Practice real-world skills with interactive scenarios.' }}"
        />
    @else
