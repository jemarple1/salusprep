...
@section('meta_title', 'Sign Up — NREMT® & NCLEX-PN™ Full Access')
@section('meta_description', 'Create your SalusPrep account for unlimited adaptive NREMT® and NCLEX-PN™ practice tests, flashcards, skill exercises, and daily mock exams.')
    ...
    <div class="mx-auto max-w-lg">
        <div class="rounded-2xl border border-safety/20 bg-navy-light/80 p-8">
            ...
            <p class="mt-2 text-sm text-slate-400">
                Choose your platform, create your account, and unlock unlimited adaptive practice.
            </p>

            <form method="POST" action="{{ route('register') }}" class="mt-6 space-y-5">
                @csrf

                <x-register-platform-picker
                    :options="$sectionOptions->all()"
                    :default-slug="$defaultSectionSlug"
                />
                ...
                <button type="submit" class="w-full rounded-xl bg-safety py-3.5 font-bold text-navy shadow-lg shadow-safety/20 hover:bg-safety-light">
                    Create account &amp; checkout — {{ $unlockPrice }}
                </button>
            </form>
